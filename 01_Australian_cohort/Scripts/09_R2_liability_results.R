...
# Convert observed R2 to the liability scale using CP prevalence 
...
r2l_function <- function(data) {
  ...
  for (i in 1:n_boot) {
    ...
    lm_full <- lm(CP_all ~ PGS_CP_FinnGen_SBayesRC + PGS_EA_SBayesRC + PGS_Birthweight_SBayesRC + 
...
r2l_df_monogenic <- r2l_function(Monogenic_only)
r2l_df_non_monogenic <- r2l_function(Non_monogenic)
r2l_df_young525 <- r2l_function(Young525)

save(r2l_df_full,
     r2l_df_monogenic,
     r2l_df_non_monogenic, 
     r2l_df_young525,
...
## ----MonoR2lPrint
r2l_df_monogenic %>% 
  kable(caption = "Variance explained in CP risk on the liability scale attributable to each PGS or all PGS in same model (individuals with a monogenic diagnosis for CP only)(as a proportion, multiply by 100 for a %)")
## ----

## ----NonmonoR2lPrint
r2l_df_non_monogenic %>% 
  kable(caption = "Variance explained in CP risk on the liability scale attributable to each PGS or all PGS in same model (individuals without a monogenic diagnosis for CP)(as a proportion, multiply by 100 for a %)")
## ----

## ----Young525R2lPrint
r2l_df_young525 %>% 
  kable(caption = "Variance explained in CP risk on the liability scale attributable to each PGS or all PGS in same model (controls restricted to youngest 525 individuals)(as a proportion, multiply by 100 for a %)")
## ----
